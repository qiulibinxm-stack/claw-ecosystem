...
import { ScanTrendingInputSchema, GenerateBriefInputSchema } from './schemas.js';
import { formatScanResultMarkdown, formatScanResultJson, formatBriefMarkdown, formatBriefJson, generateSummaryTable } from './formatter.js';
...
// ============================================
server.registerTool('github_trending_scan', {
...
// Tool 2: 生成学习简报
// ============================================
server.registerTool('github_trending_brief', {
...
- 学习方向决策`,
    ...
    try {
        ...
        if (focusKeywords && focusKeywords.length > 0) {
            allRepos.sort((a, b) => {
                const aScore = focusKeywords.filter(kw => `${a.name} ${a.description}`.toLowerCase().includes(kw.toLowerCase())).length;
                const bScore = focusKeywords.filter(kw => `${b.name} ${b.description}`.toLowerCase().includes(kw.toLowerCase())).length;
        ...
        if (categoryCounts.ai_ml > 3)
            focusAreas.push('🤖 AI/机器学习 — 多Agent编排、LLM应用');
        if (categoryCounts.devtools > 3)
            focusAreas.push('🛠️ 开发工具 — CLI、SDK、框架');
        if (categoryCounts.web > 3)
            focusAreas.push('🌐 Web开发 — 前端框架、全栈方案');
        if (categoryCounts.data > 3)
            focusAreas.push('📊 数据处理 — 分析、可视化、Pipeline');
...
// Tool 3: 快速摘要
// ============================================
server.registerTool('github_trending_summary', {
