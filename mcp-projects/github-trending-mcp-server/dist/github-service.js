...
import { GITHUB_TRENDING_URL, TIME_RANGE_MAP, REQUEST_TIMEOUT, AI_ML_KEYWORDS, DEVTOOLS_KEYWORDS, WEB_KEYWORDS, DATA_KEYWORDS } from './constants.js';
/**
 * 爬取 GitHub Trending 页面
 */
export async function fetchTrendingPage(language, timeRange) {
 ...
 * 解析 HTML 提取仓库信息
 */
export function parseTrendingRepos(html) {
...
export function categorizeRepo(repo) {
    ...
    if (AI_ML_KEYWORDS.some(kw => text.includes(kw)))
        return 'ai_ml';
    if (DEVTOOLS_KEYWORDS.some(kw => text.includes(kw)))
        return 'devtools';
    if (WEB_KEYWORDS.some(kw => text.includes(kw)))
        return 'web';
    if (DATA_KEYWORDS.some(kw => text.includes(kw)))
        return 'data';
    return 'other';
}
/**
 * 提取热门关键词
 */
export function extractKeywords(repos) {
 ...
 * 扫描 GitHub Trending
 */
export async function scanTrending(languages, timeRange, limit) {
    const results = [];
