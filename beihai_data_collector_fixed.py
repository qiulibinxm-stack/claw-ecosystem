...
class BeihaiDataCollector:
    ...
    def search_poi(self, keyword: str, city: str = "北海") -> dict:
        """搜索POI点位"""
    ...
    def get_around_pois(self, location: str, radius: int = 500) -> list:
        """获取周边POI"""
...
def main():
    ...
    success_rate = len(collector.results)/len(collector.target_locations)*100 if collector.target_locations else 0
    print(f"成功率: {success_rate:.1f}%")
    
    # 保存摘要报告
    with open("beihai_summary.txt", "w", encoding="utf-8") as f:
        f.write("北海市目标点位数据采集报告\n")
        f.write("=" * 50 + "\n")
        f.write(f"采集时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"目标点位: {len(collector.target_locations)}个\n")
        f.write(f"成功采集: {len(collector.results)}个\n")
        ...
        for result in collector.results:
            ...
            f.write(f"  坐标: {result['location']}\n")
            f.write(f"  电话: {result.get('tel', '无')}\n")
            f.write(f"  周边POI数量: {result['around_pois_count']}\n")
    
    print(f"摘要报告已保存到: beihai_summary.txt")
