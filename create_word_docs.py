...
def create_word_document(filename, title, content):
    """创建Word文档（使用纯文本格式）"""
    # 创建带有基本格式的文本文件，可以导入到Word中
    word_content = f"""<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
{content}
    ...
    # 保存为RTF文件
    with open(filename.replace('.docx', '.rtf'), 'w', encoding='utf-8') as f:
        f.write(rtf_content)
    
    # 同时保存为纯文本，可以复制到Word中
    with open(filename.replace('.docx', '_for_word.txt'), 'w', encoding='utf-8') as f:
        f.write(f"{title}\n\n{content}")
    
    return filename.replace('.docx', '.rtf')

def create_deep_research_docx():
    """创建深度调研报告Word文档"""
    print("正在创建深度调研报告Word文档...")
...
"""
    ...
    # 创建Word文档格式的文件
    rtf_file = create_word_document("酒店深度调研报告.docx", title, content)
    print(f"[完成] Word格式报告已生成：{rtf_file}")
    
    # 同时保存为格式化的文本文件
    with open("酒店深度调研报告_格式化.txt", "w", encoding="utf-8") as f:
        f.write(content)
    
    return rtf_file

def create_project_evaluation_docx():
    """创建项目评估报告Word文档"""
    print("正在创建项目评估报告Word文档...")
...
"""
    ...
    # 创建Word文档格式的文件
    rtf_file = create_word_document("酒店项目评估报告.docx", title, content)
    print(f"[完成] Word格式报告已生成：{rtf_file}")
    
    # 同时保存为格式化的文本文件
    with open("酒店项目评估报告_格式化.txt", "w", encoding="utf-8") as f:
        f.write(content)
...
def main():
    """主函数"""
    print("=" * 50)
    print("开始生成Word格式酒店分析报告...")
    print(f"时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 50)
    
    try:
        # 创建深度调研报告Word文档
        deep_research_file = create_deep_research_docx()
        
        # 创建项目评估报告Word文档
        evaluation_file = create_project_evaluation_docx()
        
        print("\n" + "=" * 50)
        print("Word格式报告生成完成！")
        print("已生成以下文件：")
        print(f"1. {deep_research_file} - RTF格式，可直接用Word打开")
        print(f"2. {evaluation_file} - RTF格式，可直接用Word打开")
        print("3. 酒店深度调研报告_格式化.txt - 格式化文本，可复制到Word")
        print("4. 酒店项目评估报告_格式化.txt - 格式化文本，可复制到Word")
        print("=" * 50)
        
        # 列出生成的文件
        files = os.listdir('.')
        report_files = [f for f in files if f.endswith(('.rtf', '_格式化.txt'))]
        print(f"\n当前目录中的Word格式文件：")
        ...
        print("\n" + "=" * 50)
        print("使用说明：")
        print("1. RTF文件：直接用Microsoft Word打开")
        print("2. 格式化文本：复制内容到Word中，保持格式")
        print("3. 所有文件都包含完整的中文格式和表格")
        print("=" * 50)
        
    except Exception as e:
        print(f"生成Word格式报告时出错: {e}")
        import traceback
        traceback.print_exc()
