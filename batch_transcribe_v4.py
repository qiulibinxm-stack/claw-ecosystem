...
def find_source_root_v2(filepath):
    """
    找文件最直接的源根：找到文件所在目录的最近祖先源目录
    比如: file=D:\东玄学习\韩少嵛真神论命真神论命法\配套资料\06.pdf
         DOC_DIRS includes D:\东玄学习\韩少嵛真神论命真神论命法\配套资料
         → 返回 D:\东玄学习\韩少嵛真神论命真神论命法\配套资料
         → relpath = 06.pdf → output = D:\东玄学习\转录文本\06.pdf
    """
    # 获取文件的父目录链
    parts = Path(filepath).parts  # (D:, 东玄学习, 韩少嵛真神论命真神论命法, 配套资料, 06.pdf)
    # 从长到短构建父目录
    for i in range(len(parts)-1, 0, -1):
        candidate = os.path.join(*parts[:i])  # 从短到长：先 D:\, 再 D:\东玄学习, ...
        if os.path.isdir(candidate):
            for src in VIDEO_DIRS + DOC_DIRS:
                try:
                    rel = os.path.relpath(candidate, src)
                    if not rel.startswith('..') and not os.path.isabs(rel):
                        # candidate 是 src 的子目录（或等于 src）
                        # 但我们想要 candidate == src 或 candidate 在 src 下的某个层级
                        pass
                except ValueError:
                    pass
...
def get_output_path(source_path, source_root):
    """输出路径 = OUTPUT_ROOT / (文件相对于source_root的路径, 扩展名统一为.txt)"""
    rel = os.path.relpath(source_path, source_root)
    # 去掉盘符里的冒号 + 统一扩展名为.txt
    name_without_ext = os.path.splitext(rel)[0]
    return os.path.normpath(os.path.join(OUTPUT_ROOT, name_without_ext + '.txt'))

def is_actually_done(output_txt):
    return os.path.exists(output_txt) and os.path.getsize(output_txt) > 200
...
def extract_audio(video_path, audio_path, start=0, duration=None):
    ...
    if r.returncode != 0:
        err = r.stderr.decode('utf-8', errors='replace')[-300:] if r.stderr else 'unknown'
...
def process_video(video_path, progress):
    source_root = find_source_root_v2(video_path)
    if not source_root:
...
def process_document(doc_path, progress):
    source_root = find_source_root_v2(doc_path)
    if not source_root:
