...
for i, pf in enumerate(pdfs):
    ...
    if os.path.exists(txt_path) and os.path.getsize(txt_path) > 100:
        print(f'[SKIP {i+1}] {txt_name}', flush=True)
        continue

    print(f'[OCR {i+1}/{len(pdfs)}] {pf}', flush=True)
    try:
        ...
        with open(txt_path, 'w', encoding='utf-8') as f:
            f.write(combined)
        print(f'[OK {i+1}] {txt_name} - {len(combined)} chars', flush=True)
    except Exception as e:
        print(f'[ERR {i+1}] {pf}: {e}', flush=True)

print('=== PDF OCR COMPLETE ===', flush=True)
